...
// Backdates leads across the last 6 months so dashboard charts (which
// group by createdAt) show a realistic spread instead of a single spike.
function buildSeedLeads(authorName) {
  const now = Date.now();
  return SAMPLE_LEADS.map((lead, i) => {
    const daysAgo = Math.floor(Math.random() * 180);
    const createdAt = new Date(now - daysAgo * 86400000);
    const notes = lead.status !== 'new'
      ? [{ content: SAMPLE_NOTES[i % SAMPLE_NOTES.length], addedBy: authorName, createdAt }]
      : [];
    return { ...lead, notes, createdAt, updatedAt: createdAt };
  });
}

module.exports = { SAMPLE_LEADS, SAMPLE_NOTES, buildSeedLeads };
